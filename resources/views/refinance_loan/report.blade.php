...
<h2 class="d-flex" style="max-width: 800px">
    <span> Quotation (Loan - Refinance) </span>
</h2>
    ...
    <table class="table sm:table-sm table-responsive-lg" id="showReportTable">
        <tbody>
            ...
            <tr class="static">
                ...
                <td>
                    <a href="{{ route('calculator-item.create', ["section" => "professional_charges", "cid" => 5]) }}" class="btn btn-primary">Add</a>
                </td>
            </tr>

            @php
            $start = 0;
            $items = \App\Models\RefinanceLoan::pro_charge_items();
            @endphp
            ...
            <tr class="static">
                ...
                <td>
                    <a href="{{ route('calculator-item.create', ["section" => "reimbursements", "cid" => 5]) }}" class="btn btn-primary">Add</a>
                </td>
            </tr>

            @php
            $start = $items->last()->pos;
            $items = \App\Models\RefinanceLoan::reimbursement_items();
            @endphp
            @foreach ($items as $i=>$item)
            ...
            <tr class="static">
                ...
                <td>
                    <a href="{{ route('calculator-item.create', ["section" => "disbursements", "cid" => 5]) }}" class="btn btn-primary">Add</a>
                </td>
            </tr>

            @php
            $start = $items->last()->pos;
            $items = \App\Models\RefinanceLoan::disbursement_items();
            @endphp
            @foreach ($items as $i=>$item)
