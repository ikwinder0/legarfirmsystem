...
@php
    ...
    }else if($current_value && is_countable($current_value) ){
        if(count($current_value) >0) {
            foreach ($current_value as $_value) {
                if (is_object($_value) && is_subclass_of(get_class($_value), 'Illuminate\Database\Eloquent\Model') ) {
                    $current_value = $_value->getKey();
                    break;
                }
            }
        }else {
