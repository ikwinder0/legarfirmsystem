    ...
    <table class="table sm:table-sm table-responsive-lg" id="showReportTable">
        <tbody>
            ...
            <tr class="static">
                ...
                <td>
                    <a href="{{ route('calculator-item.create', ["section" => "professional_charges", "cid" => 1]) }}" class="btn btn-primary">Add</a>
                </td>
            </tr>

            @php
            $start = 0;
            $items = \App\Models\Purchase::pro_charge_items();
            @endphp
            ...
            <tr class="static">
                ...
                <td>
                    <a href="{{ route('calculator-item.create', ["section" => "reimbursements", "cid" => 1]) }}" class="btn btn-primary">Add</a>
                </td>
            </tr>

            @php
            $start = $items->last()->pos ?? 0;
            $items = \App\Models\Purchase::reimbursement_items();
            @endphp
            @foreach ($items as $i=>$item)
            ...
            <tr class="static">
                ...
                <td>
                    <a href="{{ route('calculator-item.create', ["section" => "disbursements", "cid" => 1]) }}" class="btn btn-primary">Add</a>
                </td>
            </tr>

            @php
            $start = $items->last()->pos ?? 0;
            $items = \App\Models\Purchase::disbursement_items();
            @endphp
            @foreach ($items as $i=>$item)
