    ...
    <table class="table sm:table-sm table-responsive-lg" id="showReportTable">
        <tbody>
            ...
            <tr class="static">
                ...
                <td>
                    <a href="{{ route('calculator-item.create', ["section" => "professional_charges", "cid" => 2]) }}" class="btn btn-primary">Add</a>
                </td>
            </tr>

            @php
            $start = 0;
            $items = \App\Models\Loan::pro_charge_items();
            @endphp
            ...
            <tr class="static">
                ...
                <td>
                    <a href="{{ route('calculator-item.create', ["section" => "reimbursements", "cid" => 2]) }}" class="btn btn-primary">Add</a>
                </td>
            </tr>

            @php
            $start = $items->last()->pos;
            $items = \App\Models\Loan::reimbursement_items();
            @endphp
            @foreach ($items as $i=>$item)
            ...
            <tr class="static">
                ...
                <td>
                    <a href="{{ route('calculator-item.create', ["section" => "disbursements", "cid" => 2]) }}" class="btn btn-primary">Add</a>
                </td>
            </tr>

            @php
            $start = $items->last()->pos;
            $items = \App\Models\Loan::disbursement_items();
            @endphp
            @foreach ($items as $i=>$item)
